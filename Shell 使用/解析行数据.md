...
```bash
curl --json '{"command": "ALL", "orderId": $order_id, "userId"： $user_id}' "https://domain/api/.."
```
...
cat order.csv | while IFS=, read order_id user_id; do
    curl --json '{"command": "ALL", "orderId": $order_id, "userId"： $user_id}' "http://localhost:9120/api/syncOrder"
done

# 或使用管道配合 `xargs -P` 并行执行
cat order.csv | while IFS=, read -r orderId userId; do
  echo "curl --json '{\"command\": \"ALL\", \"orderId\": $orderId, \"userId\": $userId}' \
