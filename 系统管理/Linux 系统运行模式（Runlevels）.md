...
| --- | --- | --- |
| **0** | 停止模式 | 关机 |
| **1** | 命令行单用户模式 | 只允许超级用户（即 `root`）使用。**没有网络没有守护进程**，属于 **自救** 模式。 |
| **2** | 命令行多用户模式 | 同样的 **没有网络**  且 **不运行守护进程**。 |
| **3** | 命令行多用户模式 | **这个就是我们服务器使用的模式**。 |
| **4** | 自定义 | 该模式完全由用户自己去定义，除非是超级高高高手，否者没人会使用该模式 |
| **5** | X11 模式 | 这个就是桌面版 GUI 图形模式，也是桌面版 Ubuntu 及其他桌面版默认的运行模式 |
| **6** | 重启 | 这个是重启模式，注意是无限重启！除非你非常熟悉 Linux，否者不要使用该模式 |



上面表格中每一种级别都有具体的说明，而我们经常使用的命令行模式其实就是 Linux 的 runleve3，而桌面版 GUI 其实就是 Linux 的 runlevel6。
​
