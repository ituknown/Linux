...
用户组实际上就是权限控制中的某个功能模块，类似于通常系统设计中的角色、角色组。

Linux 也不例外，用户组主要是用于系统权限控制，这个在团队协作中尤为的重要。系统中的某个文件只能归属一个用户（root 除外），其他用户是没有读写权限的。

但是在实际工作中通常会分团队，比如张三和李四归属于一个团队。张三编写的设计文档上传到服务器之后李四理应能够编辑和查看的，但是该文件的归属组却是张三，那该怎么办呢？

这个时候就需要用户组了，我们只需要创建一个组，将张三和李四都添加进去，同时指定文件的所有组为创建的这个组即可。这样，不管这个文件是谁创建的，只要用户在所属组中就都能够有相应的操作权限了。

有关文件的权限以及所有者这里就不做赘述，可以参考下 [chown 命令与文件归属系统.md](chown%20命令与文件归属系统.md) 和 [chmod 命令与文件权限系统.md](chmod%20命令与文件权限系统.md)。

另外，下面的内容虽然简单，但是涉及到用户的概念，因此还是推荐你先看下 [系统用户管理](%E7%B3%BB%E7%BB%9F%E7%94%A8%E6%88%B7%E7%AE%A1%E7%90%86.md) 之后再回过头来看下面的内容。
